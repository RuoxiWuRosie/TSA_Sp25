...
Mean_seas <- meanf(y= ts_electricity_price[, "All.sectors"],
                    h = 36)
# this is a bad forcasting, since it forces a simple average on a dataset with seasonality 
plot(Mean_seas)


#Model 2: Arithmetic mean on deseas data
mean_deseas <- meanf(y = deseasonal_allsectors_price,
                     h = 36)
print(mean_deseas)

#Model 3: Seasonal naive on original data
SNAIVE <- snaive(ts_electricity_price[, "All.sectors"],
                    h = 36)
plot(SNAIVE)
#Model 4: Naive on deseas data
NAIVE <- naive(y = deseasonal_allsectors_price,
                     h = 36)
plot(NAIVE)
...
SMA_deseas <- smooth::sma(deseasonal_allsectors_price,
                          h = 36,
               holdout = FALSE,
               silent = TRUE)
plot(SMA_deseas)
...
SMA_seas_original <- smooth::sma(ts_electricity_price[, "All.sectors"],
                          h = 36,
               holdout = FALSE,
               silent = TRUE)
plot(SMA_seas_original)

# Model 7:  Simple exponential smoothing on deseasonal data


# Model 8:  Simple exponential smoothing on original data
SMA_seas_original <- ses(ts_electricity_price[, "All.sectors"],
                          h = 36,
               holdout = FALSE,
               silent = TRUE)
plot(SMA_seas_original)
...
SARMIA_seas <- auto.arima(ts_electricity_price[, "All.sectors"])
plot(SARMIA_seas)
