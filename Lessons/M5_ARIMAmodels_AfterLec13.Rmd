...
Model_111 <- Arima(deseasonal_residential_price,
                   order=c(1,1,1),
                   include.drift = TRUE)
print(Model_111)

#y_t = 0.030 + 0.6312*y_t-1 - 0.7426*a_t-1 + a_t
...
Model_011 <- Arima(deseasonal_residential_price,
                   order=c(0,1,1),
                   include.drift = TRUE)
print(Model_011)
...
Model_211 <- Arima(deseasonal_residential_price,
                   order=c(2,1,1),
                   include.drift = TRUE)
print(Model_211)
...
Now that you have played with different order, let's try the auto.arima() function from the base package *stats*. The best fit for this time series is a  with drift.

```{r}
#Auto ARIMA
Model_autofit <- auto.arima(deseasonal_residential_price, 
