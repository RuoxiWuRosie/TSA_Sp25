...
> Comments: Note a clear trend on residuals series, showing that the mean is not a good to model the trend component. And aside from trend the seasonal component is also not being modeled. ACF also show temporal dependence (seasonal). The residuals plot also did not dislay normal distribution.
...
> Comments: The residuals for the seasonal naive don't show any seasonal patterns. Recall the model repeats the observation that happen in a previous seasonal lag (in this case one year ago). The residuals series show a strong autoregressive component which is not desired. The ACF displayed a lot of significant values still and the residuals plot still does not appear very normal.
...
> Comments: This is by far the best fit. Notice the residual series seems to be random and ACF shows no significant self correlation. Just by looking at residuals, it's normal distribution as well, which means SARIMA is the model to use.
